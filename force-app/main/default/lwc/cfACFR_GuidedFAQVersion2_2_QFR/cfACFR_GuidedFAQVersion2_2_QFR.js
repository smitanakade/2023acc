...
export default class cfACFR_GuidedFAQVersion2_2_QFR extends FlexCardMixin(
	LightningElement
) {
	@api debug;
	@api recordId;
	...
	connectedCallback() {
		...
		this.registerEvents();
		this.setAttribute("class", "card-0koBm0000000KzdIAE");
		this.getCommunityPrefix().then((result) => {
			loadStyle(
				this,
				result + "/servlet/servlet.FileDownload?file=00PBm000001ZcGLMA0"
			).then(() => {
				//Loading Custom CSS.
			});
		});
	...
	performAction(event) {
		...
		if (
			event.currentTarget &&
			event.currentTarget.closest(".cf-vlocity-state")
		) {
			stateIndex =
				event.currentTarget.closest(".cf-vlocity-state").dataset.index;
			//This will be undefined in case of repeat record is off
			recordIndex =
				event.currentTarget.closest(".cf-vlocity-state").dataset.rindex;
		}
		let stateSelector = ".cf-vlocity-state[data-index='" + stateIndex + "']";
		if (recordIndex) {
			stateSelector += "[data-rindex='" + recordIndex + "']";
		}
		const flexActionSelector =
			stateSelector + " ." + dataset.actionElementClass;
		...
		if (actionList?.length) {
			...
			this.fireMultipleActionRecursively(
				eventObj,
				0,
				null,
				null,
	...
	executeAction(event) {
		...
		if (dataset && typeof dataset.elementLabel !== "undefined") {
			this.elementIndex =
				event.currentTarget && event.currentTarget.closest(".cf-vlocity-state")
					? event.currentTarget.closest(".cf-vlocity-state").dataset.rindex
					: -1;
			...
			action.actionList =
				action.actionList || (action.actionData ? [action.actionData] : []);
	...
	handleEventAction(eventObj, eventIndex, event) {
		eventObj.actionList =
			eventObj.actionList || (eventObj.actionData ? [eventObj.actionData] : []);
		...
		if (eventObj.eventtype === "event" && event?.target) {
			if (
				this.elementIndex >= 0 &&
				event?.target?.classList.contains("execute-action")
			) {
