import { FlexCardMixin } from "omnistudio/flexCardMixin";
    import {interpolateWithRegex, interpolateKeyValue, fetchCustomLabels, loadCssFromStaticResource } from "omnistudio/flexCardUtility";
          ...
          export default class cfACFR_Header extends FlexCardMixin(OmniscriptBaseMixin(LightningElement)){
              @api debug;
              @api recordId;
              @api objectApiName;
              @track _omniSupportKey = 'cfACFR_Header';
 ...
 this.registerEvents();
                this.setAttribute(
                  "class",
                  "card-0ko9p00000001qXAAQ"
                );
                this.getCommunityPrefix().then((result) => {
                  loadStyle(this, result +"/servlet/servlet.FileDownload?file=00P9p000000iRcREAU").then(() =>{
                    //Loading Custom CSS.
                  })
                });
