import { FlexCardMixin } from "omnistudio/flexCardMixin";
    ...
    import { load } from "omnistudio/newportLoader";
          ...
          export default class cfACFR_FormNameAndInformation extends FlexCardMixin(OmniscriptBaseMixin(LightningElement)){
              @api debug;
              @api recordId;
              @api objectApiName;
              @track _omniSupportKey = 'cfACFR_FormNameAndInformation';
        ...
        @api set cfFormName(val) {
          if(typeof val !== "undefined") {
            this._sessionApiVars["FormName"] = val;
          }
        } get cfFormName() {
          return this._sessionApiVars["FormName"] || "";
        }
              ...
              connectedCallback() {
                ...
                this.setAttribute(
                  "class",
                  "card-0ko9p00000005UNAAY"
                );
                this.getCommunityPrefix().then((result) => {
                  loadStyle(this, result +"/servlet/servlet.FileDownload?file=00P9p000000zulGEAQ").then(() =>{
                    //Loading Custom CSS.
                  })
                });
